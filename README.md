# MBG Freshness AI — YOLOv8 Version
...
An editorial, real-time AI-powered freshness classification web application tailored for the **Makan Bergizi Gratis (MBG)** national program. Built using **Ultralytics YOLOv8 Classification** and served via a lightweight, high-performance Flask backend with a premium, responsive web interface.
...
graph TD
    ...
    F -->|6. Feed Forward Inference| G[YOLOv8 Classify Model]
    G -->|7. Return Raw Class Probabilities| H[Piecewise Suitability Scorer]
    H -->|8. Compute Suitability % & Recommendation| I[Flask JSON Response]
    I -->|9. AJAX Success Handler| C
    C -->|10. Render Viewport & Dynamic Stats| A
```
...
4. **AI Inference:** The trained **YOLOv8 Classification** model (`runs/classify_freshness/weights/best.pt`) predicts class probabilities for `segar` and `tidak_segar`.
...
Notebook ini akan otomatis mengunduh base model `yolov8n.pt`, menginstal dependensi YOLOv8, melatih model Object Detection selama 25 epoch, dan mengekspor weights terbaik ke `runs/detect_freshness/weights/best.pt`.
